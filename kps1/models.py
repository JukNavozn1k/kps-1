...
def build_cfnn(
    ...
    *,
    ...
    for i, spec in enumerate(layers):
        if hidden_outputs:
            concat = tf.keras.layers.Concatenate(name=f"concat_{i+1}")([inputs] + hidden_outputs)
        else:
            concat = inputs
    ...
    if hidden_outputs:
        final_concat = tf.keras.layers.Concatenate(name="concat_out")([inputs] + hidden_outputs)
        out_inp = final_concat
    else:
        out_inp = inputs
